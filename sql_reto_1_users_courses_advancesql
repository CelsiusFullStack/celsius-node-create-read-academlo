drop table if exists users_relation_tmp;
CREATE TEMP TABLE users_relation_tmp AS
(SELECT DISTINCT
	users.id_user,
	users.first_name,
	users.last_name,
...
FROM
	...
	select *,
	case
	  when endeds = 0 then '0 %' 
		when endeds > 0 then  (endeds *100 ) / chapters || '%'  
	end as complete
	from users_relation_tmp
